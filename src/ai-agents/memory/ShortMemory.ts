...
import { ShortMemoryUtils } from './utils/ShortMemoryUtils.js';

export type RedisSaverParams = {
  connection: Redis;
};

export class ShortMemory extends BaseCheckpointSaver {
  connection: Redis;
  ...
  async put(
    ...
    const key = ShortMemoryUtils.makeRedisCheckpointKey(
      thread_id ?? '',
      checkpoint_ns,
      checkpoint_id,
    );
  ...
  async putWrites(
    ...
    const dumpedWrites = ShortMemoryUtils.dumpWrites(this.serde, writes);

    for (const [idx, write] of dumpedWrites.entries()) {
      const key = ShortMemoryUtils.makeRedisCheckpointWritesKey(
        thread_id,
        checkpoint_ns,
        checkpoint_id,
        task_id,
  ...
  async getTuple(config: RunnableConfig): Promise<CheckpointTuple | undefined> {
    ...
    checkpoint_id =
      checkpoint_id ??
      ShortMemoryUtils.parseRedisCheckpointKey(checkpointKey).checkpoint_id;

    const writesKey = ShortMemoryUtils.makeRedisCheckpointWritesKey(
      thread_id,
      checkpoint_ns,
      checkpoint_id,
      '*',
    ...
    const parsedKeys = matchingKeys
      .map((key) => ShortMemoryUtils.parseRedisCheckpointWritesKey(key))
      .sort((a, b) => Number(a.idx) - Number(b.idx));

    const pendingWrites = await ShortMemoryUtils.loadWrites(
      this.serde,
      Object.fromEntries(
        await Promise.all(
    ...
    return ShortMemoryUtils.parseRedisCheckpointData(
      this.serde,
      checkpointKey,
      checkpointData,
      pendingWrites,
  ...
  async *list(
    ...
    const { thread_id, checkpoint_ns } = config.configurable ?? {};
    const pattern = ShortMemoryUtils.makeRedisCheckpointKey(
      thread_id,
      checkpoint_ns,
      '*',
    );
    ...
    keys = ShortMemoryUtils.filterKeys(keys, before, limit);

    for (const key of keys) {
      const data = await this.connection.hgetall(key);
      if (data && data.checkpoint && data.metadata) {
        yield ShortMemoryUtils.parseRedisCheckpointData(this.serde, key, data);
      }
    }
  }
  ...
  private async _getCheckpointKey(
    ...
    if (checkpoint_id) {
      return ShortMemoryUtils.makeRedisCheckpointKey(
        thread_id,
        checkpoint_ns,
        checkpoint_id,
      );
    ...
    const all_keys = await this.connection.keys(
      ShortMemoryUtils.makeRedisCheckpointKey(thread_id, checkpoint_ns, '*'),
    );
    ...
    const latest_key = all_keys.reduce((maxKey, currentKey) => {
      const maxKeyData = ShortMemoryUtils.parseRedisCheckpointKey(maxKey);
      const currentKeyData =
        ShortMemoryUtils.parseRedisCheckpointKey(currentKey);
